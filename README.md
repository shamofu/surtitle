...
Run from the repository root on Windows. First obtain the native build artifact from the selected workflow run or build the recorded native recipe; see [native runtime management](docs/native-runtime.md). Native preparation consumes the reviewed source-built libmpv artifact and downloads the fixed official CPU ONNX Runtime archive. It stops if the required build or source evidence is missing.
...
On Windows, the two projects share at most two Vitest workers to leave capacity for their PowerShell, C# and nested process checks. Tests that launch external processes have explicit deadlines for each child and an overall budget for their sequential steps; ordinary UI and script tests keep Vitest's default timeout.
...
Development stays on main; do not commit until the owner permits it. GitHub Actions checks pushes to main and release, and PRs targeting either branch. A push to release publishes the workflow's packaged artifacts after its required jobs pass. Versions are changed explicitly on main; CI does not create commits or replace published artifacts.

CI queues runs for each branch or PR without canceling the active run (`queue: max`, up to 100 pending runs). Linux checks and the native build run concurrently. Windows 2025 checks and packaging run concurrently after the native build succeeds; release-branch publication waits for Linux, Windows and package checks. Job dependencies and artifact downloads connect these stages within the workflow run. Docker environment layers use separate development/native caches; host pnpm stores and job-specific Rust dependency outputs are also cached. Native builds reuse checksum-verified source downloads and matching C/C++ compiler results through ccache, with no host mounts. Configuration, linking, source audits and final artifact generation run on every invocation. Tests cover executable behavior and output validation; workflow structure assertions have been removed.
