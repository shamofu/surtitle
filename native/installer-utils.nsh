...
!macro CheckIfAppIsRunning executableName productName
  ...
  surtitle_nsis_utils::StrReplace "$(appRunning)" "{{product_name}}" "${productName}"
  Pop $R1
  surtitle_nsis_utils::StrReplace "$(appRunningOkKill)" "{{product_name}}" "${productName}"
  Pop $R2
  surtitle_nsis_utils::StrReplace "$(failedToKillApp)" "{{product_name}}" "${productName}"
  Pop $R3

  !if "${INSTALLMODE}" == "currentUser"
    surtitle_nsis_utils::FindProcessCurrentUser "${executableName}"
  !else
    surtitle_nsis_utils::FindProcess "${executableName}"
  !endif
  Pop $R0
  ${If} $R0 = 0
      ...
      kill_${UniqueID}:
        !if "${INSTALLMODE}" == "currentUser"
          surtitle_nsis_utils::KillProcessCurrentUser "${executableName}"
        !else
          surtitle_nsis_utils::KillProcess "${executableName}"
        !endif
        Pop $R0
        Sleep 500
        ${If} $R0 = 0
