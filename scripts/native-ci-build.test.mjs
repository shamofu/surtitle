...
// Each sequential child retains its own deadline; leave five seconds for fixture
// setup and assertions. onTestFinished cleanup has Vitest's separate hook budget.
...
`], { cwd: root, env, encoding: 'utf8', timeout: childTimeoutMs, windowsHide: true });
...
test.skipIf(process.platform === 'win32')('symlink cache roots, nested partial symlinks and FIFOs are rejected before any container exists', t => {
  for (const invalid of ['root-link', 'partial-link', 'fifo']) {
    ...
    if (invalid === 'fifo') {
      const mkfifo = spawnSync('mkfifo', [join(f.root, 'cache/source-cache/fifo')], { encoding: 'utf8', timeout: childTimeoutMs });
...
}, testBudget(4)); // Three wrapper runs plus the FIFO creation command.
