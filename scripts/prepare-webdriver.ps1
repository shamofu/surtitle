...
@{ runtimeVersion=$runtime.Name; driverVersion=$version; url=$url; sha256=(Get-FileHash -LiteralPath $driver -Algorithm SHA256).Hash; verifiedPublisher=$signature.SignerCertificate.Subject } | ConvertTo-Json | Set-Content -LiteralPath (Join-Path $destination 'receipt.json')
Write-Output $driver
